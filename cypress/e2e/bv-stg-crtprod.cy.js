...
describe.only('Beta Vendor Admin | Calendar | Create appointments by Clicking on the calendar| logged with Admin credentials', () => {
  ...
  it('Verify Product Name is the only required field to create a product- Admin credentials', () => {
    cy.visit('https://beta.vendor.bookr-dev.com/admin/calendar')
    cy.contains('Inventory').should('exist')
    cy.contains('Inventory').click({ force: true })
    cy.contains('Products').should('exist')
  ...
  it('Verify Product Name is the only required field to create a product- Admin credentials', () => {
    ...
    cy.get('input[placeholder = "Enter product name"]').type('Product1 create with automated cases')
    cy.contains('button', 'Save').should('exist')
    cy.contains('button', 'Save').click({ force: true })
    cy.contains('span', 'Product name is required').should('exist')
  })
