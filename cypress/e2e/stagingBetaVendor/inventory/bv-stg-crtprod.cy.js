...
it('Verify it is not possible to create a Product by filling up Price Name and already added Produce barcode - Admin credentials', () => {
  ...
  it('Verify brand name is required when creating a brand- Admin credentials', () => {
...
it('Verify category name is required when creating a category- Admin credentials', () => {
  //cy.visit('https://beta.vendor.bookr-dev.com/admin/calendar')
  cy.visit(Cypress.env("URL_BetaVendor_Staging") + 'auth')
  cy.contains('Inventory').should('exist')
  cy.contains('Inventory').click({ force: true })
...
it('Verify bookr category is required when creating a category- Admin credentials', () => {
  //cy.visit('https://beta.vendor.bookr-dev.com/admin/calendar')
  cy.visit(Cypress.env("URL_BetaVendor_Staging") + 'auth')
  cy.contains('Inventory').should('exist')
  cy.contains('Inventory').click({ force: true })
