...
const clearProductBasicInfo = () => {
    ...
    cy.get('input[placeholder="Enter product barcode"]').clear({ force: true })
    ...
    cy.get('input[placeholder="Enter short description for your product"]').clear({ force: true })
    cy.contains('span','Product description').parent('label').next('div').find('textarea').should('exist')
    cy.contains('span','Product description').parent('label').next('div').find('textarea').clear({ force: true })
}
...
const filloutProductBasicInfo = (prod_name, prod_barcode, prod_measurement, prod_short_description, prod_description) => {
  ...
  cy.contains('span','Product description').parent('label').next('div').find('textarea').should('exist')
  cy.contains('span','Product description').parent('label').next('div').find('textarea').eq(0).type(prod_description)
}
...
const expectedMessageCreateProduct = (product_message) => {
    cy.contains('h3','Update produce details').parent('div').next('div').find('button', 'Save', { matchCase: false }).should('exist')
    cy.contains('h3','Update produce details').parent('div').next('div').find('button', 'Save', { matchCase: false }).click({ force: true })
...
//Navigation within the Creation Tabs
  ...
  //Create succesfully
  it('Verify it is possible to Edit a Product by filling up only the Product Name - Admin credentials', () => {
    accessToEditProduct()
    clearProductBasicInfo()
...
//0123456789101
  ...
  it('Verify Product is Edit successfully by filling up Price Name and Product Measurement with Mililiter Unit - Admin credentials', () => {
    accessToEditProduct()
    clearProductBasicInfo()
  ...
  it.skip('Verify Product is Edit successfully by filling up Price Name and Selecting a Category Editd from the Create product form - Admin credentials', () => {
    ...
    cy.get('section').next('div').find('button').click({ force: true })
    cy.contains('span', 'Category Editd', { matchCase: false }).should('exist')
    cy.contains('label>span', 'Category', { matchCase: false }).parents('label').next('div').find('input').click({ force: true })
    cy.contains('h3', 'Categories', { matchCase: false }).should('exist')
  ...
  it('Verify Product is Edit successfully by filling up Price Name and Reorder Quantity - Admin credentials', () => {
    accessToEditProduct()
...
it('Verify it is no possible to Edit a Product by filling up Price Name and already added SKU - Admin credentials', () => {
  accessToEditProduct()
  filloutProductInventoryInfo('asdf1234567','{enter}','{enter}','{enter}')
  filloutProductBasicInfo('Product filled up with Price name and SKU','{enter}','{enter}','{enter}','{enter}')
  expectedMessageCreateProduct('Failed to Edit product')
})

it('Verify it is not possible to Edit a Product by filling up Price Name and already added Product Bar Code - Admin credentials', () => {
  accessToEditProduct()
  filloutProductBasicInfo('Product filled up with Product Name and Bar code','123456789012','{enter}','{enter}','{enter}')
  expectedMessageCreateProduct('Failed to Edit product')
})

  it('Verify Product Name is the required field by trying to Edit a product leaving empty all the fields- Admin credentials', () => {
    accessToEditProduct()
    filloutProductBasicInfo('{enter}','{enter}','{enter}','{enter}','{enter}')
    expectedMessageCreateProduct('Product name is required')
  })

  it('Verify Product Name is the only required field by tring to Edit a product, filling up barcode only - Admin credentials', () => {
    accessToEditProduct()
    filloutProductBasicInfo('{enter}','1234567890123','{enter}','{enter}','{enter}')
    expectedMessageCreateProduct('Product name is required')
  })

  it('Verify Product Name is the only required field by trying to Edit a product, filling up Product Measurement without unit only - Admin credentials', () => {
    accessToEditProduct()
    filloutProductBasicInfo('{enter}','{enter}','1234567890','{enter}','{enter}')
    expectedMessageCreateProduct('Product name is required')
  })

  it('Verify Product Name is the only required field by trying to Edit a product filling up Product Measurement with unit only - Admin credentials', () => {
    accessToEditProduct()
    filloutProductBasicInfo('{enter}','{enter}','{enter}','{enter}','{enter}')
    cy.contains('option', 'Select Unit', { matchCase: false }).should('exist')
    cy.get('select').select('l')
  ...
  it('Verify Product Name is the only required field by trying to Edit a product filling up Short Description only - Admin credentials', () => {
    accessToEditProduct()
    filloutProductBasicInfo('{enter}','{enter}','{enter}','This is a short description','{enter}')
    expectedMessageCreateProduct('Product name is required')
  })

  it('Verify Product Name is the only required field by trying to Edit a product filling up Product Description only - Admin credentials', () => {
    accessToEditProduct()
    filloutProductBasicInfo('{enter}','{enter}','{enter}','{enter}','This is a product description')
    expectedMessageCreateProduct('Product name is required')
  })

  it('Verify Product Name is the only required field by triying to Edit a product filling up Supply Price only - Admin credentials', () => {
    accessToEditProduct()
    filloutProductPricingInfo('120','{enter}')
    expectedMessageCreateProduct('Product name is required')
  })

  it('Verify Product Name is the only required field by trying to Edit a product by filling up Retail Price only - Admin credentials', () => {
    accessToEditProduct()
  ...
  it('Verify Product cannot be Edit when bar code is less than 12 digits - Admin credentials', () => {
    accessToEditProduct()
  ...
  it('Verify Product cannot be Edit when bar code is more than 12 digits - Admin credentials', () => {
    accessToEditProduct()
  ...
  it('Verify Product cannot be Edit when bar code is alphanumeric - Admin credentials', () => {
    accessToEditProduct()
