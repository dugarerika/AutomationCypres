...
Cypress.Commands.add('filloutProductBasicInfo', (prod_name, prod_barcode, prod_measurement, prod_short_description, prod_description) => {
    ...
    cy.contains('label>span', 'Product Description').parents('label').next('div').find('textarea').should('exist')
    cy.contains('label>span', 'Product Description').parents('label').next('div').find('textarea').type(prod_description)
